...
class NIRZCG(Grating):
    si_n = interp.interp1d(*zip(*[[nu2lambda(float(f))*10**9, n] for (f, n) in h.opencsv('materials/silicon_n.csv',1)]))
    si_k = interp.interp1d(*zip(*[[nu2lambda(float(f))*10**9,n] for (f, n) in h.opencsv('materials/silicon_k.csv',1)]))

    sio2_n = interp.interp1d(*zip(*[[float(f)*(10**3), n] for f,n in h.opencsv('materials/silicon_n.csv',1)]))
